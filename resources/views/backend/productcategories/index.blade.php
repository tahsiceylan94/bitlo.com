...
@section('pagetitle') Kategoriler @endsection
...
@section('content')
    <div class="box-body">
        <div align="right">
            <a href="{{route('productcategory.create')}}"><button class="btn btn-success">Ekle</button></a>
        </div>
        <table class="table table-striped">
            ...
            <tbody id="sortable">
                @foreach($data['productcategories'] as $key)
                    <tr id="item-{{$key->id}}">
                        <td class="sortable">@if($key->file != "") <img src="/images/productcategories/{{$key->file}}" width="50"> @else {!! $key->settings_value !!} @endif</td>
                        <td>
                            <b>{{$key->title}}</b>
                        </td>
                        <td>
                            @php
                                $parent_id = \App\Models\Productcategories::where('id',intval($key->id))->where('status','1')->first();
                                if($parent_id->parent_id != null){
                                    $pid = \App\Models\Productcategories::where('id',intval($parent_id->parent_id))->where('status','1')->first();
                                    echo $pid->title;
                                }
                            @endphp
                        </td>
                        ...
                        <td><a href="{{route('productcategory.edit',$key->id)}}"><i class="fa fa-edit"></i></a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <script type="text/javascript">
            ...
            $('#sortable').sortable({
                ...
                update: function (event, ui) {
                    ...
                    $.ajax({
                        data: data,
                        type: 'POST',
                        url: '{{route('productcategory.sortable')}}',
        ...
        $('.removeline').click(function () {
            ...
            function () {
                $.ajax({
                    type: "DELETE",
                    url: "productcategory/"+destroy_id,
                    success: function (msg) {
                        if(msg){
