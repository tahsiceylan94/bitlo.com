...
@section('pagetitle') İletişim Bilgileri @endsection

@section('breadcrumb')
    <ol class="breadcrumb">
        <li><a href="{{route('zeplin.index')}}"><i class="fa fa-envelope"></i> İletişim</a></li>
    </ol>
@endsection
...
@section('content')
    <div class="box-body">
        <div align="right">
            {{--<a href="{{route('contact.create')}}"><button class="btn btn-success">Ekle</button></a>--}}
        </div>
            ...
            <tbody id="sortable">
                @foreach($data['contacts'] as $key)
                    <tr id="item-{{$key->id}}">
                        ...
                        <td class="sortable valueArea">@if($key->type=='file') <img src="/images/contact/{{$key->value}}" height="70" /> @else {!! $key->value !!} @endif</td>
                        <td class="sortable">@if($key->delete == 1)<a id="{{$key->id}}" href="javascript:void(0);" class="removeline"><i class="fa fa-trash-o"></i></a>@endif</td>
                        <td class="sortable text-center"><a href="{{route('contact.edit',$key->id)}}"><i class="fa fa-edit"></i></a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
