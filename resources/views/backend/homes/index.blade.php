...
@section('pagetitle') Anasayfa Kutular @endsection

@section('breadcrumb')
    <ol class="breadcrumb">
        <li><a href="{{route('zeplin.settings')}}"><i class="fa fa-home"></i> Anasayfa Kutular</a></li>
    </ol>
@endsection
...
@section('content')
    <div class="box-body">
        <div align="right">
            <a href="{{route('home.create')}}"><button class="btn btn-success">Ekle</button></a>
        </div>
        <table class="table table-striped">
            ...
            <tbody id="sortable">
                @foreach($data['homes'] as $key)
                    <tr id="item-{{$key->id}}">
                        <td class="sortable">@if($key->file != "") <img src="/images/homes/{{$key->file}}" height="70"> @else {!! $key->settings_value !!} @endif</td>
                        <td>{{$key->title}}</td>
                        ...
                        <td><a href="{{route('home.edit',$key->id)}}"><i class="fa fa-edit"></i></a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
