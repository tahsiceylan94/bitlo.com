...
@section('pagetitle') Bloglar @endsection

@section('breadcrumb')
    <ol class="breadcrumb">
        <li><a href="{{route('zeplin.settings')}}"><i class="fa fa-paper-plane"></i> Bloglar</a></li>
    </ol>
@endsection
...
@section('content')
    <div class="box-body">
        ...
        <table class="table table-striped">
            ...
            <tbody id="sortable">
                @foreach($data['blogs'] as $key)
                    <tr id="item-{{$key->id}}">
                        <td class="sortable">@if($key->file != "") <img src="/images/blogs/{{$key->file}}" width="50"> @else {!! $key->settings_value !!} @endif</td>
                        <td>{{$key->title}}</td>
                        <td>
                            @foreach(\App\Models\Blogcats::all()->where('status','1') as $k) @if($key->cat == $k->id) {{$k->title}} @endif @endforeach
                        </td>
                        ...
                        <td><a href="{{route('blog.edit',$key->id)}}"><i class="fa fa-edit"></i></a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
