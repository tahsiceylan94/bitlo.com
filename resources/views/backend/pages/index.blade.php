...
@section('breadcrumb')
    <ol class="breadcrumb">
        <li><a href="{{route('zeplin.settings')}}"><i class="fa fa-file-text-o"></i> Sayfalar</a></li>
    </ol>
@endsection
        ...
        <table class="table table-striped">
            ...
            <tbody id="sortable">
                @foreach($data['pages'] as $key)
                    <tr id="item-{{$key->id}}">
                        <td class="sortable">@if($key->file != "") <img src="/images/pages/{{$key->file}}" width="50"> @else {!! $key->settings_value !!} @endif</td>
                        <td>{{$key->title}}</td>
                        ...
                        <td>@if($key->header == 1) <i class="fa fa-check-square var" title="yayınlanıyor"></i> @elseif($key->header == 2) <i class="fa fa-calendar-check-o "></i> @else <i class="fa fa-times-rectangle yok" title="yayınlanmıyor"></i> @endif</td>
                        <td>@if($key->footer == 1) <i class="fa fa-check-square var" title="yayınlanıyor"></i> @elseif($key->footer == 2) <i class="fa fa-calendar-check-o "></i> @else <i class="fa fa-times-rectangle yok" title="yayınlanmıyor"></i> @endif</td>
                        <td><a id="{{$key->id}}" href="javascript:void(0);" class="removeline"><i class="fa fa-trash-o"></i></a></td>
                        <td><a href="{{route('page.edit',$key->id)}}"><i class="fa fa-edit"></i></a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
