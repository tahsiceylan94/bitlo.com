...
@section('pagetitle') Haberler Genel Ayarlar @endsection

@section('breadcrumb')
    <ol class="breadcrumb">
        <li><a href="{{route('zeplin.index')}}"><i class="fa fa-newspaper-o"></i> Haberler Genel Ayarlar</a></li>
    </ol>
@endsection
...
@section('content')
    <div class="box-body">
        <div align="right">
            {{--<a href="{{route('newsoption.create')}}"><button class="btn btn-success">Ekle</button></a>--}}
        </div>
        <table class="table table-striped">
            ...
            <tbody id="sortable">
                @foreach($data['newsoptions'] as $key)
                    <tr id="item-{{$key->id}}">
                        ...
                        <td class="sortable valueArea">@if($key->type=='file') <img src="/images/newsoption/{{$key->value}}" height="70" /> @else {!! $key->value !!} @endif</td>
                        <td class="sortable">@if($key->delete == 1)<a id="{{$key->id}}" href="javascript:void(0);" class="removeline"><i class="fa fa-trash-o"></i></a>@endif</td>
                        <td class="sortable text-center"><a href="{{route('newsoption.edit',$key->id)}}"><i class="fa fa-edit"></i></a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
